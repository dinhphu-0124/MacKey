...
void SendBackspace() {
  ...
  if (IS_DOUBLE_CODE(vCodeTable)) { // VNI or Unicode Compound
    // _syncKey có thể bị .clear() bởi RequestNewSession() (click chuột) từ
    // một sự kiện khác trước đó, khiến rỗng khi tới đây — .back()/.pop_back()
    // trên vector rỗng là undefined behavior, thường crash. Cùng pattern
    // guard này đã có sẵn ở nơi khác trong file (nhánh pData->extCode == 2).
    if (!_syncKey.empty()) {
      if (_syncKey.back() > 1) {
...
void SendShiftAndLeftArrow() {
  ...
  if (IS_DOUBLE_CODE(vCodeTable)) { // VNI or Unicode Compound
    // Xem giải thích guard tương tự ở SendBackspace().
    if (!_syncKey.empty()) {
      if (_syncKey.back() > 1) {
        if (!(vCodeTable == 3 && containUnicodeCompoundApp(FRONT_APP))) {
          CGEventTapPostEvent(_proxy, eventVkeyDown);
 ...
 * MAIN Callback.
 */
// RAII guard cho MacKeyStateLock/Unlock: khóa lúc tạo, tự mở khóa lúc hủy
// (khi ra khỏi scope theo bất kỳ điểm return nào). Dùng C++ RAII vì
// MacKeyCallback có rất nhiều điểm return sớm, tự lock/unlock thủ công
// rất dễ sót.
struct MacKeyStateLockGuard {
  MacKeyStateLockGuard() { MacKeyStateLock(); }
  ~MacKeyStateLockGuard() { MacKeyStateUnlock(); }
};

CGEventRef MacKeyCallback(CGEventTapProxy proxy, CGEventType type,
                           CGEventRef event, void *refcon) {
  // macOS tự tắt event tap khi callback xử lý chậm (timeout) hoặc khi có
  // app khác bật Secure Input. Phải bật lại ngay, nếu không MacKey sẽ mất
  // khả năng gõ vĩnh viễn cho tới khi khởi động lại app.
  if (type == kCGEventTapDisabledByTimeout ||
      type == kCGEventTapDisabledByUserInput) {
    [MacKeyManager reEnableEventTap];
    return event;
  }

  // Khóa state toàn cục của engine (vLanguage, vCodeTable, ...) trong suốt
  // quá trình xử lý phím này, để đồng bộ với các thay đổi cài đặt từ UI.
  MacKeyStateLockGuard _stateLockGuard;
