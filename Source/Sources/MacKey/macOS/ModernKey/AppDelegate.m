...
-(void)askPermission {
    ...
    [alert setMessageText: [NSString stringWithFormat:@"MacKey cần bạn cấp quyền để có thể hoạt động!"]];
...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification {
    ...
    // 1. Không tắt app nếu /Applications/MacKey.app đang chạy
    ...
    // phải dùng đúng API tra theo bundle identifier)
    NSArray<NSRunningApplication *> *runningApp =
        [NSRunningApplication runningApplicationsWithBundleIdentifier:OPENKEY_BUNDLE];
    if (runningApp.count > 1) { //đã có 1 instance khác (ngoài chính mình) đang chạy -> exit
        [NSApp terminate:nil];
        return;
    }
    ...
    if (!dontCheckUpdate)
        [MacKeyManager checkNewVersion:nil callbackFunc:nil];
...
#ifdef DEBUG
    NSMenuItem* dbgHeader = [theMenu addItemWithTitle:@"🛠 MacKey (Bản Debug - Xem UI)" action:nil keyEquivalent:@""];
...
-(void)setRunOnStartup:(BOOL)val {
    CFStringRef appId = (__bridge CFStringRef)@"com.dinhphu.MacKeyHelper";
...
-(void)onQuickConvert {
    if ([MacKeyManager quickConvert]) {
        if (!convertToolDontAlertWhenCompleted) {
            [MacKeyManager showMessage: nil message:@"Chuyển mã thành công!" subMsg:@"Kết quả đã được lưu trong clipboard."];
        }
    } else {
        [MacKeyManager showMessage: nil message:@"Không có dữ liệu trong clipboard!" subMsg:@"Hãy sao chép một đoạn text để chuyển đổi!"];
    }
}
...
#pragma mark Reset MacKey after mac computer awake
-(void)receiveWakeNote: (NSNotification*)note {
    [MacKeyManager initEventTap];
}

-(void)receiveSleepNote: (NSNotification*)note {
    [MacKeyManager stopEventTap];
}
