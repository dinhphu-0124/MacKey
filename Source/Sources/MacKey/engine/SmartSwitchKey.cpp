...
void initSmartSwitchKey(const Byte* pData, const int& size) {
    ...
    for (int i = 0; i < count; i++) {
        // Dữ liệu đã lưu (NSUserDefaults) có thể bị hỏng/cắt cụt. Phải kiểm
        // tra biên trước mỗi lần đọc, nếu không sẽ đọc tràn ra ngoài buffer.
        if (cursor + 1 > total) break;
        bundleIdSize = pData[cursor++];
        if (cursor + (Uint32)bundleIdSize + 1 > total) break;
